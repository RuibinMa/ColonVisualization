    ...
    */
    ...
    for(int iter = 0; iter < MaxIter + modify; iter++)
    {
        ...
        for(vtkIdType i=0; i<model->GetNumberOfPoints(); i++)
        {
            ...
            if(1 || violationNum > 0 && iter == MaxIter + modify - 1)
            {
                appendFilter->RemoveAllInputs();
    ...
    IllCutCirclesActor->GetProperty()->SetLineWidth(5);
                                                     ...
                                                     vtkSmartPointer<vtkDoubleArray> RefDirections, FileManager *t_filemanager)
{
    ...
    for(int i=0; i<model->GetNumberOfPoints(); i++)
    {
        ...
        else if(choice == 1)
        {
            ...
            double x = cos(6*3.1415926*p)*sqrt(2)/2;
            double y = sqrt(1-x*x);
        ...
        else if(choice == 5)
        {
            ...
            {
                double p = (S->GetValue(200) - S->GetValue(99))/(S->GetValue(200) - S->GetValue(99));
                tangent[0] = cos(3.1415926*(1.0/4.0)*p);
                tangent[1] = sin(3.1415926*(1.0/4.0)*p);
                tangent[2] = 0;
            }
            else
            {
                double p = (S->GetValue(i) - S->GetValue(99))/(S->GetValue(200) - S->GetValue(99));
                tangent[0] = cos(3.1415926*(1.0/4.0)*p);
                tangent[1] = sin(3.1415926*(1.0/4.0)*p);
                tangent[2] = 0;
            }
